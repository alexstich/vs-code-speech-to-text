...
function activate(context) {
    ...
    try {
        ...
        const errorMessage = `Failed to activate SpeechToTextWhisper: ${error.message}`;
...
function refreshContext() {
    ...
    try {
        ...
        const context = contextManager.getContext();
        vscode.window.showInformationMessage(`🔄 Context refreshed: ${context.contextType} in ${context.ideType}`);
    }
    catch (error) {
...
function resetConfiguration() {
    vscode.window.showWarningMessage('This will reset all SpeechToTextWhisper settings to defaults. Continue?', 'Yes', 'No').then(selection => {
        if (selection === 'Yes') {
            const config = vscode.workspace.getConfiguration('speechToTextWhisper');
...
function showWelcomeMessage() {
    ...
    if (!hasApiKey) {
        vscode.window.showInformationMessage('🎤 Welcome to SpeechToTextWhisper! Please configure your OpenAI API key to get started.', 'Open Settings').then(selection => {
...
function initializeContextManager() {
    ...
    contextManager = new ContextManager_1.ContextManager(contextEvents);
...
function adaptToContext(context) {
    try {
        ...
        if (statusBarManager) {
            ...
            if (context.contextType === ContextManager_1.ContextType.TERMINAL || context.contextType === ContextManager_1.ContextType.DEBUGGER) {
                // StatusBarManager уже настроен, но можем адаптировать tooltip
            }
        }
        // Для Cursor - готовимся к интеграции с чатом
        if (context.ideType === ContextManager_1.IDEType.CURSOR && context.contextType === ContextManager_1.ContextType.CHAT) {
